...
let jstv_args =
  ...
  let mk_conf source target emit_graph = { source; target; emit_graph } in

  Term.(const mk_conf $ source_arg $ target_arg $ emit_graph_arg)
...
let main () =
  ...
  let { source; target; emit_graph } = parse_command_line () in
